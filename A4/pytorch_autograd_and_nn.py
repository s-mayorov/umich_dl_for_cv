...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
    ...
    conv_w1 = nn.init.kaiming_normal_(
        torch.empty((channel_1, 3, kernel_size_1, kernel_size_1), dtype=dtype, device=device, requires_grad=True))
    ...
    fc_w = torch.zeros((num_classes, channel_2 * H * W), dtype=dtype, device=device, requires_grad=True)
...
class ThreeLayerConvNet(nn.Module):
    def __init__(self, in_channel, channel_1, channel_2, num_classes):
        ...
        ############################################################################
        # Replace "pass" statement with your code
        self.conv1 = nn.Conv2d(in_channel, channel_1, kernel_size=5, padding=2, padding_mode="zeros")
        self.conv2 = nn.Conv2d(channel_1, channel_2, kernel_size=3, padding=1, padding_mode="zeros")
...
def initialize_three_layer_conv_part4():
    ...
    # Replace "pass" statement with your code
    model = nn.Sequential(
        nn.Conv2d(C, channel_1, kernel_size=kernel_size_1, padding=pad_size_1, padding_mode="zeros"),
        nn.Conv2d(channel_1, channel_2, kernel_size=kernel_size_2, padding=pad_size_2, padding_mode="zeros"),
        Flatten(),
...
class ResidualBlock(nn.Module):
    def __init__(self, Cin, Cout, downsample=False):
        ...
        elif Cin != Cout and downsample:
            self.shortcut = nn.Conv2d(Cin, Cout, kernel_size=1, stride=2)
        else:
            raise ValueError("Error in parameters, please check sizes!")
############################################################################
        #                                 END OF YOUR CODE                         #
        ############################################################################
