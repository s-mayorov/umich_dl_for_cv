...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for epoch in range(100):
        ...
        pred, pred_idx = output.max(0)
        target = output[target_y]
        ...
        target.backward()
        with torch.no_grad():
            X_adv += learning_rate * X_adv.grad/(X_adv.grad*X_adv.grad).sum().item()
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    target = output[target_y] - l2_reg*(img*img).sum()
    target.backward()
