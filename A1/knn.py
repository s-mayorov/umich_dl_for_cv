...
def compute_distances_two_loops(x_train, x_test):
  ...
  x_train_v = x_train.reshape(num_train, -1)
  x_test_v = x_test.reshape(num_test, -1)
  for train_idx, train_example in enumerate(x_train_v):
    for test_idx, test_example in enumerate(x_test_v):
      dists[train_idx, test_idx] = (train_example - test_example).pow(2).sum().item()
...
def compute_distances_one_loop(x_train, x_test):
  ...
  x_test_v = x_test.reshape(num_test, -1)
  for train_idx, train_example in enumerate(x_train_v):
    dists[train_idx] = (x_test_v - train_example).pow(2).sum(dim=1)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = -2 * (x_train_flattened @ x_test_flattened.t()) + torch.sum(x_test_flattened ** 2, dim=1) + torch.sum(
    x_train_flattened ** 2, dim=1).reshape(-1, 1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for sample in range(num_test):
    vals, idxs = torch.topk(dists[:, sample], k=k, dim=0, largest=False)
    y_pred[sample] = torch.mode(y_train[idxs])[0]
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      ...
      train_X = torch.cat([x_train_folds[j] for j in range(num_folds) if j != i], dim=0)
      train_Y = torch.cat([y_train_folds[j] for j in range(num_folds) if j != i], dim=0)

      classifier = KnnClassifier(train_X, train_Y)
      k_to_accuracies[k].append(classifier.check_accuracy(val_X, val_Y, k=k))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k in k_to_accuracies.keys():
    mean_acc = torch.mean(torch.tensor(k_to_accuracies[k]))
