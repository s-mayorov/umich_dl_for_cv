...
def multiples_of_ten(start, stop):
  ...
  assert start <= stop
  x = None
  if stop - start < 10: return torch.tensor([], dtype=torch.float64)
...
def reverse_rows(x):
  ...
  # Replace "pass" statement with your code
  y = torch.zeros_like(x)
  y = x[list(reversed(range(x.shape[0])))]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y = x.view(2,3, -1).permute(1,0,2).contiguous().view(3,8)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
