...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dw = x_re.t() @ dout
        db = dout.sum(dim=0, keepdim=True).squeeze(0)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return dx, dw, db
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        h1, dh1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, dh2 = Linear.forward(h1, self.params['W2'], self.params['b2'])
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
        ...
        dh2, dw2, db2 = Linear.backward(dloss, dh2)
        dx, dw1, db1 = Linear_ReLU.backward(dh2, dh1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for idx, hidden_size in enumerate(hidden_dims, 1):
            self.params[f'W{idx}'] = weight_scale * torch.randn(input_size, hidden_size, device=device, dtype=dtype)
            self.params[f'b{idx}'] = torch.zeros(hidden_size, device=device, dtype=dtype)
            input_size = hidden_size

        self.params[f'W{idx + 1}'] = weight_scale * torch.randn(input_size, num_classes, device=device, dtype=dtype)
        self.params[f'b{idx + 1}'] = torch.zeros(num_classes, device=device, dtype=dtype)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for l in range(1, self.num_layers):
            h, d = Linear_ReLU.forward(inp, self.params[f'W{l}'], self.params[f'b{l}'])
            inp = h
            cache.append(d)
        scores, d = Linear.forward(inp, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        ...
        # Replace "pass" statement with your code
        loss, dloss = softmax_loss(scores, y)
        loss += self.reg * sum([torch.sum(torch.pow(x, 2)) for k, x in self.params.items() if k.startswith('W')])
        dx, dw, db = Linear.backward(dloss, d)
        grads[f'W{self.num_layers}'] = dw + 2*self.reg*self.params[f'W{self.num_layers}']
        grads[f'b{self.num_layers}'] = db
        for l in range(self.num_layers - 1, 0, -1):
            dx, dw, db = Linear_ReLU.backward(dx, cache[l - 1])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    # Replace "pass" statement with your code
    solver = Solver(
        model=model, data=data_dict,
        update_rule=sgd,
        optim_config={
            'learning_rate': 3e-2,
        },
        lr_decay=0.99,
        num_epochs=60, batch_size=200,
        print_every=500,
        device='cuda')
    ##############################################################################
    #                             END OF YOUR CODE                               #
...
def get_five_layer_network_params():
    ...
    # Replace "pass" statement with your code
    weight_scale = 15e-2  # Experiment with this!
    learning_rate = 5e-2  # Experiment with this!
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def rmsprop(w, dw, config=None):
    ...
    cache = dr * cache + (1 - dr) * (dw.pow(2))
    w += -lr * dw / (cache.sqrt() + eps)
    config['cache'] = cache
    next_w = w
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
class Dropout(object):
    ...
    @staticmethod
    def forward(x, dropout_param, device):
        """
